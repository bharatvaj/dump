...
struct job{
    const char *command;
    const char *info;
    int (*function)(node::Node *, int, char**);
    int opt_length;
    option *options;
};
...
int sh_process(int jlen, job *jobs, node::Node *n, char *line){
int count = -1;
  ssize_t arg_buffsize = 64; //chage variably
	...
	for (int i = 0; i < jlen; i++) {
        	if (strcmp(*args, jobs[i].command) == 0) {
            		return (jobs[i].function)(n, count, ++args);
        	}
	}
...
int sh_next(int jlen, job *jobs, const char *_shell_name){
    ...
    return sh_process(jlen, jobs, nullptr, buffer);
   }
